...
_RZZ_RE = re.compile(
    r"(办公室|档案馆|党办|财务部|人事科|信息中心|宣传科|审计科|技术科|保密办)"
)
...
async def run(
    *, db: AsyncSession, ctx: CapabilityContext, query: str
    ...
    similar = await svc.retrieve(
        query=fields.get("TM", query)[:60], kb_type="meta", top_k=3, filt=filt
    )
    citations = [
        {
            "chunk_id": c.chunk_id,
    ...
    lines.append("")
    lines.append(
        f"已生成编目 Patch #{str(patch.id)[:8]}（pending），等待人工审核通过后归档。"
    )
