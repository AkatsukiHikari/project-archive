<template>
  ...
  >
    <div class="px-6 py-4 border-b border-[var(--semi-color-border)]">
      <h2 class="font-semibold text-[var(--semi-color-text-0)]">基本资料</h2>
    ...
    <div v-else class="p-6 flex flex-col gap-5">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input v-model="form.full_name" placeholder="请输入真实姓名" />
        </div>
        <div>
      ...
      <div>
        ...
        <Input v-model="form.job_title" placeholder="请输入职位名称" />
      </div>
      <div>
      ...
      <!-- 操作栏 -->
      <div
        class="flex items-center gap-3 pt-4 mt-1 border-t border-[var(--semi-color-border)]"
      >
        <Button theme="solid" type="primary" :loading="saving" @click="save"
          >保存修改</Button
        >
...
import { Button, Input, TextArea, Spin, Toast } from "@kousum/semi-ui-vue";
